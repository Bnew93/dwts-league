...
export default async function TeamPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { couples, events, episodes, claims, standings, scores } = await loadSeason(league);
    ...
    return (
      <Shell ctx={ctx}>
        ...
        <div className="glass fade-up mt-5 p-6 text-center text-silver-300">
          ...
          <Link href={`/l/${slug}`} className="text-gold-300 underline decoration-gold-400/50">
            See the standings
          </Link>
        </div>
      </Shell>
    ...
    <Shell ctx={ctx}>
      ...
      {!drafted && (
        <div className="glass fade-up mt-5 p-5 text-center text-sm text-silver-300">
          ...
          <Link href={`/l/${slug}/draft`} className="text-gold-300 underline decoration-gold-400/50">
            Draft room
          </Link>
        </div>
      )}
      ...
      {out.length > 0 && (
        <section className="mt-7">
          ...
          <ul className="stagger space-y-2">
            {out.map((c) => (
              <li key={c.id}>
                <CoupleRow couple={c} owner={mySlot} slug={slug} note={null} />
              </li>
            ))}
          </ul>
        </section>
